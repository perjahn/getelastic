...
curl -s -X POST -H "$accept" -H "$auth" -H "$apiversion" "$baseurluploads/releases/$RELEASE_ID/assets?name=elasticsearch_9_3_1.tar.gz" --data-binary "@elasticsearch_9_3_1.tar.gz" -H "$contenttype" > /dev/null
curl -s -X POST -H "$accept" -H "$auth" -H "$apiversion" "$baseurluploads/releases/$RELEASE_ID/assets?name=filebeat_9_3_1.tar.gz" --data-binary "@filebeat_9_3_1.tar.gz" -H "$contenttype" > /dev/null
